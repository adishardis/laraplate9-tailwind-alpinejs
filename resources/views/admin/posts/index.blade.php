<x-admin-base-layout>
  {{-- Page Title --}}
  <x-page-title :title="__('Post')" :role="'Admin'" />
  {{-- End Page Title --}}
  ...
  <div class="w-full mb-12 xl:mb-0 px-4">
    <x-table-simple :id="'posts-table'" :columns="[
        ...
        ]" :urlData="'/admin/fetch/posts?mode=datatable'">
      <x-slot name="linkColumn">
        <div class="flex flex-wrap space-x-4">
          <x-edit-action url='posts' id='row.id' isTextId="true" />
        </div>
      </x-slot>
      <x-slot name="actionColumn">
        <div class="flex flex-wrap space-x-4">
          <x-crud-action url='{{ route("admin.posts.index") }}' id="row.id" isEdit="true" isDelete="true" />
        </div>
      </x-slot>
