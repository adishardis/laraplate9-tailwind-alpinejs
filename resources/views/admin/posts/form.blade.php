<x-admin-base-layout>
  {{-- Page Title --}}
  <x-page-title :title="(isset($post) ? 'Edit' : 'Create').' POST'" :role="'Admin'" />
  {{-- End Page Title --}}
  ...
  <x-slot name="navHeader">
    <x-navbar-breadcrumbs :items="[
      ...
      [
        ...
        'url' => (isset($post) ? route('admin.posts.edit', compact('post')) : route('admin.posts.create'))
      ]
    ]" />
  </x-slot>
  {{-- End Nav Header --}}
        ...
        <div class="flex flex-wrap">
          ...
          <div class="w-full px-4">
            <div class="relative w-full mb-3">
              ...
              <div x-data="{
                url: '/admin/fetch/posts?mode=post-status',
          ...
          <div class="flex justify-end">
            <div class="py-6 px-3 mt-32 sm:mt-0">
              <a class="bg-indigo-500 active:bg-indigo-600 uppercase text-white font-bold hover:shadow-md shadow text-xs px-4 py-2 rounded outline-none focus:outline-none sm:mr-2 mb-1 ease-linear transition-all duration-150"
                type="button" href="{{ route('admin.posts.index') }}">
